 ...
 */
export const startDisplay = function (canvas) {
    // Globals
    let spinning = true
    let showHelp = false
    ...
    /**
     * Project an point to screen
     * @param {number} x
     * @param {number} y
     * @param {number} z
     */
    const toScreen = function ({ x, y, z }) {
        return {
     ...
     * @param {number} z
     */
    const RotX = function (angle, { x, y, z }) {
        return {
            x: x,
    ...
    const rotY = function (angle, { x, y, z }) {
        return {
    ...
    const transZ = function ({ x, y, z }, dz) {
        return {
            x: x,
            y: y,
            z: z + dz,
    ...
    function frame() {
        ...
        for (const vertex of vertices) {
            const v = RotX(angX, rotY(angY, vertex))
            drawPoint(toScreen(transZ(rotY(spinAngle, v), moveZ)))
        }
        ...
        for (const face of faces) {
            for (let i = 0; i < face.length; i++) {
                ...
                const aScreen = toScreen(transZ(rotY(spinAngle, RotX(angX, rotY(angY, a))), moveZ))
                const bScreen = toScreen(transZ(rotY(spinAngle, RotX(angX, rotY(angY, b))), moveZ))
        ...
        /// The classic ms between frames == 1 second / FPS
